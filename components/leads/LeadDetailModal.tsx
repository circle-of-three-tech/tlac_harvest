...
export default function LeadDetailModal({
  ...
  // Only fetch if we have a real server ID (CUIDs start with 'c' and are 25 chars;
  // fallback IDs generated offline use crypto.randomUUID() format with dashes).
  useEffect(() => {
    const id = lead?.id;
    if (!id) return; // no id at all — offline/unsaved lead, skip fetch
    // UUIDs generated by crypto.randomUUID() contain hyphens; real Prisma CUIDs don't
    const looksLikeServerCuid = /^c[a-z0-9]{24,}$/.test(id);
    if (!looksLikeServerCuid) return; // offline temp ID, skip fetch
    if (lead.addedBy?.phone && lead.assignedTo?.phone) return; // already have phone data

    fetch(`/api/leads/${id}`)
      .then(r => {
        if (!r.ok) return null; // don't crash on 404/403 etc.
        return r.json();
      })
      .then(data => {
        if (data && data.id && data.fullName) {
          // Only update if response looks like a valid lead, not an error object
          setLead(data);
        }
      })
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-1 sm:gap-2 mt-0.5 flex-wrap sm:flex-nowrap">
          ...
          <div className="md:col-span-2 space-y-4 sm:space-y-6">
            {!editing ? (
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className={`border-t border-harvest-100 pt-3 sm:pt-4 ${role === "EVANGELIST" ? "hidden" : "block"}`}>
